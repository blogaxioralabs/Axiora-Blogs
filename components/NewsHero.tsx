...
export function NewsHero({ news }: { news: any }) {
    if (!news) return null;
    ...
    // Tags අරගෙන මුල් 3 විතරක් වෙන් කිරීම
    const displayTags = Array.isArray(news.tags) ? news.tags.slice(0, 3) : [];
        ...
        <section className="mb-8 md:mb-12">
            ...
            >
                ...
                <div className="absolute top-4 left-4 md:top-6 md:left-6 flex flex-wrap items-center gap-2 md:gap-3 z-20">
                    ...
                    <span className="flex items-center gap-1.5 bg-white text-black px-3 py-1 md:px-4 md:py-1.5 rounded-full text-[10px] md:text-xs font-bold shadow-md">
                            ...
                            <div className="flex flex-wrap items-center gap-3 md:gap-4 text-white/90 text-xs md:text-sm font-medium">
                                ...
                                <div className="flex items-center gap-1.5 text-rose-300">
                                    ...
                                    <span className="uppercase tracking-wider text-[10px] md:text-xs font-bold">
                                        {news.like_count || 0} Likes
                                    </span>
                                </div>
                            </div>
                        </div>

                        {/* Right Side: Tags (දකුණු පැත්තේ යටින්ම 3ක් විතරක්) */}
