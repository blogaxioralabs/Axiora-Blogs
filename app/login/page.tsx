...
import { useEffect, useState } from 'react'; // <-- useState එකතු කරන්න
...
export default function LoginPage() {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        setLoginError(false); // Reset error state on new submission
        const { error } = await supabase.auth.signInWithPassword({ email: values.email, password: values.password });
        if (error) {
            toast.error(error.message);
            // --- Error එකක් ආවොත් state එක true කරනවා ---
            setLoginError(true);
            // ---------------------------------------
        } else {
    ...
    const handleOAuthSignIn = async (provider: 'google') => { /* ... */
        setLoginError(false); // Reset error state on OAuth attempt
        ...
        <div className="flex min-h-screen w-full items-center justify-center bg-background p-4">
            ...
            <motion.div className="w-full max-w-md space-y-6 rounded-lg border bg-card p-6 sm:p-8" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div className="text-center">
                    ...
                    <p className="text-balance text-muted-foreground">Enter your email below to access your dashboard.</p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    {/* Password */}
                    <div>
                        {/* --- Forgot Password link එක මෙතනින් අයින් කළා --- */}
                        {/* <div className="flex justify-between items-baseline"> ... </div> */}
                        {/* ------------------------------------------- */}
                        <Label htmlFor="password" className="sr-only">Password</Label> {/* Label එක තියන්න */}
                        <div className="relative">
                    ...
                    {/* --- Forgot Password Link (Conditional Rendering) --- */}
                    {loginError && (
                ...
                {/* OAuth Separator */}
                 ...
                 <div className="grid grid-cols-2 gap-4">
                     ...
                     <TooltipProvider><Tooltip><TooltipTrigger asChild><Button variant="outline" disabled className="w-full"><AppleIcon />Apple</Button></TooltipTrigger><TooltipContent><p>Coming soon!</p></TooltipContent></Tooltip></TooltipProvider>
                 </div>
